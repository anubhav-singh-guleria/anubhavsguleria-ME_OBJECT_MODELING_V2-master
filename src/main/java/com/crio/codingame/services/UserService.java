...
public class UserService implements IUserService {
    ...
    public User create(String name) {
        ...
        // newUser.setId(Integer.toString(++id));
        return newUser;
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
        ...
        else return userRepository.findAll().stream()
        .sorted((p1,p2)->-(p1.getScore().compareTo(p2.getScore())))
        .collect(Collectors.toList());
    }
    ...
    public UserRegistrationDto withdrawContest(String contestId, String userName) throws ContestNotFoundException, UserNotFoundException, InvalidOperationException {
        ...
        if((Object)contest==Optional.empty())throw new ContestNotFoundException("Contest not found");
        Optional<User> user = userRepository.findByName(userName);
        if((Object)user==Optional.empty())throw new UserNotFoundException("User Not Found");
        User existingUser = user.get();
        Contest existingContest = contest.get();
        if(existingContest.getContestStatus()==ContestStatus.ENDED || existingContest.getContestStatus()==ContestStatus.IN_PROGRESS) throw new InvalidOperationException(existingContest.getContestStatus().name());
        if(existingContest.getCreator().equals(existingUser))throw new InvalidOperationException("User is the contest creator");
        if(!existingUser.checkIfContestExists(existingContest)) throw new InvalidOperationException("User not in the given contest");
        existingUser.deleteContest(existingContest);
        userRepository.save(existingUser);
     return new UserRegistrationDto(existingContest.getName(), userName, RegisterationStatus.NOT_REGISTERED);
    }
